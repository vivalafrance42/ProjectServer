...
{
    public class JwtHandler(IConfiguration configuration, UserManager<FoodPlaceMenuItemsUser> userManager)
    {
        // generates the token
        public async Task<JwtSecurityToken> GetTokenAsync(FoodPlaceMenuItemsUser user) =>
        new(
        ...
        private async Task<List<Claim>> GetClaimsAsync(FoodPlaceMenuItemsUser user)
        {
